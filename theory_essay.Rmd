...
@burtt2004 found that dark pluamge is resistant to bacterial degradation (~ pigments), a common problem in humid climes; methods: measure intrasp. ∆colour vs bacterial activity <!--cf @Koski2015 -> UV role (in plants)-->.

@tate2016 found persistent colour polymorphism within populations of black sparrowhawk (*Accipiter melanoleucus*) -> equivalent fitness of the morphs in heterogeneous habitats; results: darker species forage/hunt better in darker habitats (~ hiding in the ambient background) -> support for the "light level-detectability hypothesis" (Galeotti et al. 2003). Since (Ruan says) darker habs are wetter (ish), this relates to Gloger's rule.
...
These cases exemplify the way in which different mechanisms and the combinations and varying strengths thereof can all produce patterns in accrodance with Gloger's rule. Each specific case need not display a Gloger's rule pattern as a consequence of all, nor some specific process [@Olalla2011]. Thus, the extent of a Gloger's rule pattern seems to be that which arises due to multiple processes.
